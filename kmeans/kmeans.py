...
def loadData(fname):#加载数据，并返回读取的所有数据点的x，y值
...
def generatePoint(x,y,k):#找到k个初始点：先随机数据点中的一个作为第一个初始点，再在剩余点中找到与之距离最远的点作为第二个初始点，以此类推找到k个初始点
...
def findFarPoint(x, y):#计算数据中所有点与当前质点组距离和，并返回距离最大的点的坐标
    for i in range(len(x)):
        distance[i] = sum(findDistance(x[i], y[i], pointX, pointY))
    j = distance.index(max(distance))
    return x[j], y[j]

def findDistance(x, y, pointX, pointY):#计算传入的点（x,y）与当前质点组中各个质点的距离，并一并存入一个列表中返回
...
def kmeans(fname, k):
    ...
    while times < 12:
        ...
        for i in range(len(x)):#对每个数据点，都计算与当前质点组的距离，找到距离最近的质点，并将该数据点归属到该质点所在的簇
